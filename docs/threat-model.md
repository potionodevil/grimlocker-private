...
└── AND Attack vector 4: Clipboard/screen capture
    ├── Clipboard history                 → MITIGATED: No copy/paste of keys; no clipboard usage
...
| Clipboard/screenshot capture | Medium | Medium | **Medium** | No copy/paste; blocked screenshots |
...
| Enhancement | Would Address | Priority |
|---|---|---|
| **SGX/SEV enclave** | Kernel-level memory access, DMA attacks | **High** (Enterprise) |
| **Reproducible builds** | Supply chain trust | **High** |
| **Argon2id for coordinate keys** | Memory-hardness for override path | **Medium** |
| **YubiKey/SoloKey integration** | Phishing resistance, hardware-bound keys | **Medium** (SDK ready) |
| **FDE key destruction** | Guaranteed data destruction on SSDs | **Medium** |
| **Enhanced eBPF/seccomp filter** | Reduce kernel attack surface | **Low** |
