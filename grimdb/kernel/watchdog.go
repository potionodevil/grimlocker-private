...
func NewWatchdog(bus Dispatcher, registry *Registry, appDir string) *Watchdog {
	return &Watchdog{
		bus:              bus,
		registry:         registry,
		appDir:           appDir,
...
func (w *Watchdog) heartbeatLoop(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			...
			_ = w.bus.Dispatch(NewEvent("watchdog", EvSecAudit, []byte(`{"level":"info","module":"watchdog","message":"heartbeat_tick"}`)))
		}
	}
}
...
func (w *Watchdog) checkVaultStatus(ctx context.Context) {
	...
	_ = w.bus.Dispatch(NewEvent("watchdog", EvAuthInitReady, initPayload))
}
...
func (w *Watchdog) vfsAutoMountLoop(ctx context.Context) {
	unsubscribe := w.bus.Subscribe(EvAuthResult, func(ev Event) error {
		var payload struct {
		...
		vfsMountEv := NewEvent("watchdog", EvStorageVFSMount, []byte(`{"action":"mount"}`))
		...
		go func() {
			...
			readyEv := NewEvent("watchdog", EvStorageReady, readyPayload)
...
func (w *Watchdog) healthCheckLoop(ctx context.Context) {
	unsubscribe := w.bus.Subscribe(EvSystemHealthCheck, func(ev Event) error {
		...
		reply := ReplyEvent("watchdog", EvSystemLog, ev, payload)
