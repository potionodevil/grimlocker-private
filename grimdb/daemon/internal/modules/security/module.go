...
import (
	...
	dsec "github.com/grimlocker/grimdb/daemon/internal/security"
)
...
type Module struct {
	mu       sync.RWMutex
	lockdown engine_sec.LockdownManager
	audit    engine_sec.AuditLog
	guard    engine_sec.MemoryGuard
	...
	session     *engine_sec.SessionContext
...
func NewModule(cfg engine_sec.LockdownConfig, entropyPath string) *Module {
	m := &Module{
		audit:       engine_sec.NewAuditLog(1024),
		guard:       dsec.NewMemoryGuard(),
		mvkHandles:  make(map[string][]byte),
		entropyPath: entropyPath,
		exitFunc:    os.Exit,
	}
	cfg.OnHard = m.hardLockdownCallback
	m.lockdown = engine_sec.NewLockdownManager(cfg)
	return m
}
...
func (m *Module) SetSession(s *engine_sec.SessionContext) {
	m.session = s
}
...
func (m *Module) Start(ctx context.Context, d kernel.Dispatcher) error {
	...
	m.audit.Append(engine_sec.SecurityEvent{Level: engine_sec.LevelInfo, Module: moduleID, Message: "security module started"})
	return nil
}
...
func (m *Module) buildHandlers() map[kernel.EventType]secHandlerFn {
	...
	return map[kernel.EventType]secHandlerFn{
		...
		kernel.EvSecPanic: func(e kernel.Event) error {
			m.audit.Append(engine_sec.SecurityEvent{Level: engine_sec.LevelCritical, Module: moduleID, Message: "PANIC event received — triggering hard lockdown"})
...
func (m *Module) handleSecAudit(e kernel.Event) error {
	var ev engine_sec.SecurityEvent
	if err := json.Unmarshal(e.Payload, &ev); err != nil {
		// Fallback: treat plain-string payloads as simple info messages.
		m.audit.Append(engine_sec.SecurityEvent{Level: engine_sec.LevelInfo, Module: moduleID, Message: string(e.Payload)})
		return nil
	}
	m.audit.Append(ev)
	return nil
...
func (m *Module) Lockdown() engine_sec.LockdownManager { return m.lockdown }

// Audit returns the AuditLog.
func (m *Module) Audit() engine_sec.AuditLog { return m.audit }
